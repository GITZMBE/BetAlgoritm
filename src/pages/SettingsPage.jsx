...
/**
 * Settings page — bankroll entry, API key management, and strategy explainers.
 */
export function SettingsPage({ bankroll, setBankroll, onChangeApiKey, remainingReqs, isDemo }) {
    ...
    <div className="space-y-4">
      ...
      <div className="card px-6 py-5">
        ...
        <p className="text-sm text-sub font-body leading-relaxed mb-5">
          Your total betting budget. Used to calculate exactly how much to stake on each bet using
        ...
        {bankroll > 0 && (
          <>
            <p className="section-label">
              Example stakes from {formatCurrency(bankroll)} bankroll
            </p>
        ...
        <div className="flex flex-wrap gap-4 items-center">
          <p className="text-sm text-sub font-body flex-1">
            {isDemo
              ...
              : `✅ Connected · ${remainingReqs ?? "?"} requests remaining this month.`}
          </p>
          <button onClick={onChangeApiKey} className="btn-ghost">
            {isDemo ? "Connect API key" : "Change API key"}
          </button>
        </div>
      </div>
      ...
      {/* ── Strategy guide ── */}
      <div className="card px-6 py-5">
        <h2 className="text-xl font-display font-extrabold text-white mb-4">
        ...
        <div className="space-y-4">
          {EXPLAINERS.map((item, i) => (
            <div
              key={item.title}
              className={`pb-4 ${i < EXPLAINERS.length - 1 ? "border-b border-border" : ""}`}
            >
